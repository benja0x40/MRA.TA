...
# is used to accelerate the background walkthrough algorithm.
# (see normalizeArrayData function below)
# -----------------------------------------------------------------------------.
# USAGE NOT DOCUMENTED
# -----------------------------------------------------------------------------.
...
# execution of the background walkthrough algorithm.
# (see normalizeArrayData function below)
# -----------------------------------------------------------------------------.
# USAGE NOT DOCUMENTED
# -----------------------------------------------------------------------------.
...
# DETAIL ----------------------------------------------------------------------.
# Utility function for fast neighbor selection during the background walkthrough
...
# FUNCTION mean.shift
# DETAIL ----------------------------------------------------------------------.
# Core function of the background walkthrough algorithm based on a pre-computed
...
backgroundBiasEstimation <- function(x, y, AM.scale.compensation=T, smoothness=0.08, epsilon=0.001, nsteps=11, plots=F, xlab='A', ylab='M') {
  ...
  if(plots) {
    # MA-plot 1 (overlayed with a visualization of the background walkthrough)
    plot(
      x, y, xlab=xlab, ylab=ylab, pch='.', col=rgb(0,0,0,0.1),
      main="Background walkthrough"
    )
    # plot.kdtree.border(kdt)
    legend(
      "topleft",
      c("start/end points", "meanshift radius", "trajectory", "estimated bias"),
      fill=c(rgb(1,0,0), rgb(0,1,0), rgb(1,0.5,0), rgb(0.5,0,1)),
      bty='n'
    )
  }
  ...
  # Stage 2: background walkthrough using meanshift iterations
  bgwalk <- mean.shift(
  ...
  # Stage 3: estimate bias slope as a trimmed mean of the walkthrough angles
  ra <- rank(alpha)
  ...
  if(plots) {
    ...
    points(alpha/pi*180,type='l', col=rgb(1,0.5,0), lwd=1.5)
...
enrichmentQuality <- function(x, y, q.best=0.015, q.worst=5E-3, plots=F) {
  ...
  if(plots) {
    ...
    legend("topright", "worst", fill="red", bty='n')
  }
...
plotProbeDistanceControls <- function(x, rnk, dis, dlim=c(-2000, 2000), QF=NULL, n.q=100, ylab="4C") {
  ...
  plot(0, type='n', xlim=dlim, ylim=range(x, na.rm=T), xlab='probe distance', ylab=ylab, main='restriction site')
